...
public class EqualsToChain implements CriteriaChain {

    private CriteriaChain criteriaChain;

    public EqualsToChain(CriteriaChain criteriaChain) {
        ...
        if(value != null && !value.equals("")) {
            ...
            for (Annotation annotation : annotations) {
                if (annotation.annotationType() == SearchField.class) {
                    ...
                    if (annotationField.type() == SearchType.EQUAL_TO) {
                        cr = new Criteria(!annotationField.target_field().isEmpty() ? annotationField.target_field() : field.getName()).contains(value.toString());
                        criteria.and(cr);
                    }
                }
            }
